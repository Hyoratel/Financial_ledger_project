...
월별 수입/지출을 간편하게 관리할 수 있는 **모바일 중심 가계부 앱**입니다.  
깔끔한 UI, 편리한 거래 입력, 월별 요약, 카테고리별 통계 기능을 제공합니다.
IPhone 12pro(390\*844) 기준 사이즈로 제작되었습니다.
...
| 버전명      | 설명                                                   | 브랜치   | 태그        | 기간                     | 주요 변경사항 요약                                                             |
| ----------- | ------------------------------------------------------ | -------- | ----------- | ------------------------ | ------------------------------------------------------------------------------ |
| v1.0.0-team | 팀 프로젝트 최종 통합 버전                             | release2 | v1.0.0-team | 2025.04.07 \~ 2025.04.11 | 회원가입, 거래등록 및 수정,삭제 , 통계 시각화 , 프로필 설정 기능 구현 완료     |
| v2.0.0-dev  | 개인 프로젝트 고도화 시작 (모달 UI, 통계 기능 추가 등) | main     | v2.0.0-dev  | 2025.04.14 \~ 2025.05.09 | 거래 모달 개선, 월별(캘린더) 거래 기능 추가, 중복 내용 제거 및 상태관리 재구성 |
| v2.0.1      | 개인 고도화 안정화                                     | main     | v2.0.1      | 2025.05.09 \~ 2025.05.15 | alert -> pop-up 변경(UI 통일)                                                  |
...
│   ├── TransactionItem.vue             ✅ 거래 리스트 내 리스트 아이템
│   ├── TransactionList.vue             ✅ 거래 전체 리스트
...
| 회원가입    | 아이디, 비밀번호, 이메일, 휴대폰 번호 입력<br>- 아이디 중복 검사<br>- 비밀번호 유효성 검사<br> (3종류 이상 조합 시 8자리 이상 / 2종류 이상 조합 시 10자리 이상)<br>- 인증번호 불일치 또는 30초초 경과 시 만료 처리<br>- 필수 입력값 미입력 시 다음 단계 진행 불가 | Composition API, vue-router, axios, pinia, Bootstrap5, json-server, CryptoJS, 타이머 |
...
| 기능      | 설명                                                                                                                                                                               | 기술스택                                               |
| --------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| 거래 입력 | 날짜, 금액, 카테고리, 메모 입력 후 저장<br>- 음수 및 0 입력 불가<br>- 타입, 카테고리, 금액 미입력 시 저장 불가<br>- 타입 미선택 시 카테고리 선택 불가<br>- 타입 기본값 지출 설정정 | Composition API, vue-router, pinia, axios, json-server |
...
| 거래 필터링      | 날짜/기간별 거래 필터링 기능 제공(거래내역 유무에 따른 별도 화면 구성)                           | Composition API, vue-router, axios, components, pinia, Bootstrap5, json-server |
